"""
WARNING: MODIFIED FROM NATE'S VERSION!!
...
class TraitCovariance:
    ...
    N(0, tau2 L^{-1} L^{-T})` and `e ~ N(0, sigma2 I)`, then `y ~ N(0, tau2 Z
    L^{-1} L^{-T} Z^T + sigma2 I)`
    """
    ...
    def __init__(self, ts, observed_individuals=None, mutation_rate=1.0):
        """
        L = (I - E)^T diag(1/sqrt(D))
        where
        D = mutation_rate * edge_area
        """
        # ts = operations.split_upwards(tree_sequence) # should be already done
        if observed_individuals is None:
            observed_individuals = np.arange(self.Z.shape[0])
        assert (np.min(observed_individuals >= 0)
                and np.max(observed_individuals) < ts.num_individuals)
        mutational_target = mutation_rate * (ts.edges_right - ts.edges_left) * \
            (ts.nodes_time[ts.edges_parent] - ts.nodes_time[ts.edges_child])
        self.dim = len(observed_individuals)
        self.full_dim = ts.num_individuals
        self.kernel_dim = ts.num_edges
        self.observed_individuals = observed_individuals
    ...
    def __call__(self, sigma2, tau2, y_obs):
        """
        Matrix-vector product: returns
        (tau2 * Z_o (L^T)^{-1} L^{-1} Z_o^T  + sigma2 I) y ,
        where Z_o is the rows of Z corresponding to observed_individuals
        and since L = T^{-1} sqrt(1/D), this is
        (tau2 * Z_o T D T^T Z_o^T  + sigma2 I) y 
        """
        # TODO can we do this without forming num_edges x y.shape[1] vectors -- Peter thinks yes
        assert self.L.has_sorted_indices
        assert len(y_obs) == len(self.observed_individuals)
        is_vector = y_obs.ndim == 1
        if is_vector: y_obs = y_obs.reshape(-1, 1)
        # instead of doing Z[observed_individuals,:].T @ y_obs we insert zeros into y
        y = np.zeros((self.Z.shape[0], y_obs.shape[1]))
        y[self.observed_individuals, :] = y_obs
        Zy = self.Z.T @ y
        ...
        Zy = (self.Z @ Zy)[self.observed_individuals,:] * tau2 + y_obs * sigma2
        if is_vector: Zy = Zy.squeeze()
        return Zy

    def cov_all_inds(self, tau2, y_obs):
        """
        Matrix-vector product: returns
        (tau2 * Z (L^T)^{-1} L^{-1} Z_o^T) y ,
        where Z_o is the rows of Z corresponding to observed_individuals
        """
        assert self.L.has_sorted_indices
        assert len(y_obs) == len(self.observed_individuals)
        ...
        y = np.zeros((self.Z.shape[0], y_obs.shape[1]))
        y[self.observed_individuals, :] = y_obs
        # note that *.reshape(-1,1) is idempotent
        Zy = self.Z.T @ y.reshape(-1, 1)
        Zy = self.backward_solve(self.L.indptr, self.L.indices, self.L.data, Zy)
        Zy = self.forward_solve(self.L.indptr, self.L.indices, self.L.data, Zy)
        Zy = self.Z @ Zy * tau2
        if is_vector: Zy = Zy.squeeze(1)
        return Zy

    def solve(self, sigma2, tau2, b, preconditioner=None, maxitt=None, atol=0., rtol=1e-5):
        """
    ...
    def as_matrix(self, sigma2, tau2):
        """
        For testing only
        """
        assert self.L.has_sorted_indices
        ...
        Sigma = tau2 * Zd @ Sigma + sigma2 * np.eye(self.dim)
        return Sigma

    def simulate(self, sigma2, tau2, rng):
        """
        Returns edge effects, genetics values, observed values
        """
        u = rng.normal(size=(self.kernel_dim, 1))
        r = self.forward_solve(self.L.indptr, self.L.indices, self.L.data, u).flatten() * np.sqrt(tau2)
        x = self.Z @ r
        e = rng.normal(size=self.full_dim) * np.sqrt(sigma2)
        return r, x, x + e
...
class NystromPreconditioner:
    ...
    def logdet(self, sigma2, tau2):
        S = (self.D * tau2 + sigma2) / (np.min(self.D) * tau2 + sigma2)
        return np.sum(np.log(S))

    def grad_sigma2(self, sigma2, tau2, y=None):
        """
        return trace(solve(preconditioner, deriv(preconditioner, sigma2)))
        or if vectors `y` are passed:
        return preconditioner @ deriv(preconditioner, sigma2) @ y
        """
        spectrum = self.D * tau2 + sigma2
        norm = np.min(spectrum)
        spectrum /= norm
        grad_sigma2 = (1 - spectrum) / spectrum / norm
        if y is None:
            return np.sum(grad_sigma2)
        assert y.ndim == 2
        control = (self.U.T @ y) * grad_sigma2[:, np.newaxis]
        return self.U @ control

    def grad_tau2(self, sigma2, tau2, y=None):
        """
        return trace(solve(preconditioner, deriv(preconditioner, tau2)))
        or if vectors `y` are passed:
        return preconditioner @ deriv(preconditioner, tau2) @ y
        """
        spectrum = self.D * tau2 + sigma2
        norm = np.min(spectrum)
        spectrum /= norm
        grad_tau2 = (self.D - np.min(self.D) * spectrum) / spectrum / norm
        if y is None:
            return np.sum(grad_tau2)
        assert y.ndim == 2
        control = (self.U.T @ y) * grad_tau2[:, np.newaxis]
        return self.U @ control
