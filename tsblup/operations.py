...
def _split_node_up(n, position, ts, tables, nodes_edge, edges_left):
    ...
    while edge_id != tskit.NULL:
        ...
        er = position
        if el < er:
            tables.edges.add_row(left=el, right=er, parent=edge.parent, child=edge.child)
...
def split_upwards(ts):
    ...
    for ((left, right), edges_out, edges_in) in ts.edge_diffs(include_terminal=True):
        for edge in edges_out:
            ...
            if el < edge.right:
                tables.edges.add_row(left=el, right=edge.right, parent=edge.parent, child=edge.child)
