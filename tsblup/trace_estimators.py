...
def hutchinson(A: Callable, N: int, m: int, rng: np.random.Generator = None) -> [float, float]:
    """
    Hutchinson from https://arxiv.org/pdf/2301.07825
    `N` is dimension of operator `A` and `m` is the number of random vectors
    """
...
def xtrace(A: Callable, N: int, m: int, rng: np.random.Generator = None) -> [float, float]:
    """
...
def xnystrace(A: Callable, N: int, m: int, rng: np.random.Generator = None) -> [float, float]:
    """
...
def xdiag(A: Callable, N: int, m: int, rng: np.random.Generator = None, At: Callable = None) -> np.ndarray:
    """
    XDiag from https://arxiv.org/pdf/2301.07825
    Assumes A is self-adjoint if `At` is not supplied
    `N` is dimension of operator `A` and `m` is the number of random vectors
    """
    if rng is None: rng = np.random.default_rng()
    if At is None: At = A  # assumes self-adjoint
    m = int(np.floor(m / 2))
    cnormc = lambda M: M / np.linalg.norm(M, 2, axis=0)[np.newaxis, :]  # divide by column norm
    diag_prod = lambda A, B: np.sum(A * B, axis=0)  # diag(A' @ B)
    ...
    Y = A(Om)
    Q, R = np.linalg.qr(Y)
    Z = At(Q)  # Z = A.T @ Q -- we're assuming A is self-adjoint
    T = Z.T @ Om
    S = cnormc(np.linalg.inv(R).T)
