...
@jitclass(spec_parallel)
class TraitVectorParallel:
    def __init__(
        self,
        num_nodes,
        samples,
        ...
        self.stack = np.zeros((N, num_traits), dtype=np.float64)
        self.NULL = -1 # to avoid tskit.NULL in numba
        self.random_alloc = np.zeros(num_traits, dtype=np.float64)
    ...
    def get_z(self, u):
        ...
        if p == self.NULL or u >= self.virtual_root:
            return np.zeros(self.num_traits, dtype=np.float64)
        time = self.nodes_time[p] - self.nodes_time[u]
        span = self.position - self.x[u]
        for i in range(self.num_traits):
            self.random_alloc[i] = np.random.standard_normal()
        return np.sqrt(time * span) * self.random_alloc
    ...
    def run(self):
        ...
        out = np.zeros((len(self.samples), self.num_traits))
